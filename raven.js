...
//========================================================================================================================//		      
case "save": {
    if (!m.quoted?.isStatus) {
        return client.sendMessage(m.chat, {
            react: { text: "❌", key: m.key },
            text: "Oops!\nReply to a status update to archive it 📥",
            ephemeral: true
        }, { quoted: m });
    }
    ...
    try {
        ...
        const filename = BLACKY-STATUS-${timestamp}.blacky;
        
        // 🎨 Prepare premium message
        const statusInfo = `
╔═══✦✧✦═══╗
  🅢🅣🅐🅣🅤🅢 🅐🅡🅒🅗🅘🅥🅔
╚═══✦✧✦═══╝
...
👤 Original Poster: @${m.quoted.sender.split('@')[0]}
...
`.trim();
        ...
        // 📤 Forward to user's DM with premium formatting
        await client.sendMessage(m.sender, {
            text: statusInfo,
            mentions: [m.quoted.sender, m.sender]
        });
        
        // 🎭 Forward the actual status (discreetly)
        await client.forwardMessage(m.sender, m.quoted, {
            sendEphemeral: true,
            fileName: filename,
            caption: "🔒 Secured Status Content"
        });

        // 💫 Cool interactive confirmation
        await client.sendMessage(m.chat, {
            react: { text: "✅", key: m.key },
            text: *✨ Status Archived!*\n\n +
                  ...
                  📛 Unique ID: ${filename}\n +
                  ⏳ Timestamp: ${timestamp}\n\n +
                  _Use /statuslist to view all_,
            ephemeral: true
        }, { quoted: m });

        // 🔔 Optional: Notify owner (toggleable)
        if (NOTIFY_OWNER) {
            await client.sendMessage(ownerNumber, {
                text: 📥 New Status Save\n\n +
                      By: @${m.sender.split('@')[0]}\n +
        ...
        await client.sendMessage(m.chat, {
            ...
            text: *Archive Failed!*\n\n +
                  ...
                  Please message me first @${botNumber.split('@')[0]},
            mentions: [botNumber]
        });
    }
    break;
